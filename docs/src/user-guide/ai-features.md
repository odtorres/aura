...
In Insert mode, the AI shows ghost text suggestions after the cursor. These are generated by the speculative engine when it detects relevant suggestions for the current context.
...
Accepted text is attributed to the AI author in the CRDT authorship tracking.

## AI Refactoring

`:refactor <instruction>` sends a refactoring task to the AI via the chat panel:

```
:refactor rename UserService to AccountService everywhere
:refactor extract the validation logic into a separate function
:refactor convert all callbacks to async/await
```

The AI responds in the chat panel with proposed changes.

## AI Code Review

`:review` sends the current staged git diff to the AI for code review. The AI provides feedback on:
...
Results appear in the chat panel.

## AI Pair Programming

`:pair on` enables pair programming mode where the AI actively monitors your edits and proactively suggests:

- Test cases after writing a function
- Documentation for public APIs
- Error handling after `unwrap()` calls
- Simplifications for complex logic

Toggle off with `:pair off`.

## Conversation Export

`:export [path]` exports the chat conversation as a markdown file:

```
:export                     " → aura-chat-export.md
:export review-notes.md     " → review-notes.md
```

Each message includes the role (You / AURA AI / System), timestamp, and content.
